...
export const GET: APIRoute = async ({ url }) => {
  try {
    console.log("🎯 Player search index API called!");
    const urlParams = url.searchParams;
    const shard = parseInt(urlParams.get("shard") || "1"); // Default to shard 1

    console.log(`📡 Request URL: ${url.toString()}`);
    console.log(`📊 Parameters: shard=${shard}`);
    ...
    console.log(
      `✅ Successfully loaded search index shard ${shard} with ${data.players?.length || 0} players`,
    );
