...
async function init() {
  ...
  const cmpTypeSel = document.getElementById(
    "cmpType",
  ) as HTMLSelectElement | null;
  const comparisonType = ((cmpTypeSel?.value as any) ||
    getParam(PARAM.type, "")) as "trinket" | "race" | "";
  ...
  const sortBy = getParam(
    PARAM.sort,
    comparisonType === "trinket" ? "percent" : "dps",
  );
  ...
  if (!comparisonType || !classSlug || !specSlug) {
    ...
    if (chartEl)
      chartEl.innerHTML =
        '<div class="card"><div>Select a comparison type, class and spec to see comparisons.</div></div>';
    return;
  }
  ...
  if (!resp.ok) {
    ...
    renderError(
      errorEl,
      `Error loading data: ${resp.status} ${resp.statusText}`,
      metaEl,
      chartEl,
      loadingEl as any,
    );
  ...
  const fmt = {
    formatDuration:
      (window as any).WoWConstants?.formatDuration ||
      ((s: number) => `${Math.floor(s / 60)}m ${s % 60}s`),
    formatSimulationDate:
      (window as any).WoWConstants?.formatSimulationDate ||
      ((d: any) => String(d)),
    formatRaidBuffs: utilFormatRaidBuffs,
  };
  const label = comparisonType === "trinket" ? "Trinket" : "Race";
  const CLASS_COLORS: Record<string, string> =
    (window as any).WoWConstants?.CLASS_COLORS || {};
  const barColor = CLASS_COLORS[classSlug] || "#666";

  if (metaEl)
    metaEl.innerHTML = renderComparisonMetadataHTML(
      data.metadata,
      classSlug,
      specSlug,
      label as any,
  ...
  if (chartEl) {
    chartEl.innerHTML = renderComparisonChartHTML(
      data,
      sortBy,
    ...
    chartEl
      .querySelectorAll(".chart-item-wrapper .chart-item-header")
      .forEach((el) => {
...
if (!(window as any).__simCompInitDone) {
  ...
  try {
    __initSimulationComparison();
  } catch (e) {
    console.error("[sim] comparison init error", e);
  }
}
