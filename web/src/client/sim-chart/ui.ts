...
export function updateComparisonUI(comparisonType: string) {
  try {
    ...
    const percentOpt = sortSel ? sortSel.querySelector('option[value="percent"]') as HTMLOptionElement | null : null;
    if (percentOpt) {
      percentOpt.style.display = comparisonType === 'trinket' ? '' : 'none';
      if (comparisonType !== 'trinket' && sortSel && sortSel.value === 'percent') sortSel.value = 'dps';
      if (comparisonType === 'trinket' && sortSel && (!sortSel.value || sortSel.value === 'dps')) sortSel.value = 'percent';
    }
  } catch (e) {
