...
var analyzeCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if strings.TrimSpace(periodsCSV) != "" {
			...
			for i := a; i <= b; i++ {
				periodParts = append(periodParts, fmt.Sprintf("%d", i))
			}
...
func runAnalyze(client *blizzard.Client, realms map[string]blizzard.RealmInfo, dungeons []blizzard.DungeonInfo, periodsSpec string, outPath, statusDir string) error {
			...
			for res := range results {
				...
				if maxTs > cur.MostRecent {
					...
					for _, m := range latestRun.Members {
						nm := ""
						if v, ok := m.GetPlayerName(); ok {
							nm = v
						}
						sid := 0
						if v, ok := m.GetSpecID(); ok {
							sid = v
						}
						rslug := ""
						if v, ok := m.GetRealmSlug(); ok {
							rslug = v
						}
				...
				if _, ok := coverage[key]; !ok {
					coverage[key] = map[string]map[string]agg{}
				}
				if _, ok := coverage[key][res.Dungeon.Slug]; !ok {
					coverage[key][res.Dungeon.Slug] = map[string]agg{}
				}
	...
	for key, byDungeon := range coverage {
		...
		for slug := range byDungeon {
			dns = append(dns, dn{slug: slug, name: dName(slug)})
		}
		...
		for _, d := range dns {
			...
			if latestTs > 0 {
				ds.LatestISO = time.UnixMilli(latestTs).UTC().Format("2006-01-02 15:04:05 UTC")
			}
	...
	sort.Slice(realmStatuses, func(i, j int) bool {
		if realmStatuses[i].Region == realmStatuses[j].Region {
			return realmStatuses[i].RealmName < realmStatuses[j].RealmName
		}
		return realmStatuses[i].Region < realmStatuses[j].Region
	})
